# This python file is adapted from https://github.com/bigcode-project/bigcode-evaluation-harness/blob/main/bigcode_eval/utils.py
...
class TokenizedDataset(IterableDataset):
    """Tokenize and preprocess the dataset Multiple copies of the same prompt are sent
    sequentially. See compute_code for more details.
    ...
    def _make_instruction_prompt(self, instruction, context, prefix=""):
        """Make a prompt for instruction-tuning.

        Delimit instruction and context with specific tokens if provided.
        """
...
def update_code_gens(
    ...
    for sample, generated_tokens in gen_token_dict.items():
        for s in generated_tokens:
            ...
            code_gens[sample].append(task.postprocess_generation(gen_code, int(sample)))
    return code_gens
