...
Flower Servers are very customizable, but for simple workloads, we can start a server using the `start_server <https://flower.ai/doc/framework/ref-api/flwr.server.start_server.html>`_ function and leave all the configuration possibilities at their default values, as seen below.
