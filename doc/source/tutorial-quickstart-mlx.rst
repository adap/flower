...
In this federated learning tutorial we will learn how to train simple
...
found in the ``get_params()`` and ``set_params()`` functions. Indeed,
MLX doesn't provide an easy way to convert the model parameters into a
list of ``np.array`` objects (the format we need for the serialization
of the messages to work).
...
an identical signature to that of ``client_fn()``, but the return type
is `ServerAppComponents
...
.. note::
   ...
   <https://github.com/adap/flower/blob/main/examples/quickstart-mlx>`_
   of the extended version of this tutorial in
   ``examples/quickstart-mlx`` in the Flower GitHub repository.
