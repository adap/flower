...
**Cluster.** Flower telemetry assigns a random in-memory cluster ID each time a Flower workload starts. This enables us to understand which device types are not just starting a flower workload but also successfully completing it.
...
You may delete the source ID at any time. If you wish for all events logged under a specific device ID to be deleted, you can send a deletion request mentioning the source ID to `telemetry@flower.dev`. All events related to that device ID will then be permanently deleted.
