...
> [!IMPORTANT]
> Please note that `[tool.flwr.app.config.static]` are not allowed to be modified for fair competition if you plan to participated in the [LLM leaderboard](https://flower.ai/benchmarks/llm-leaderboard). Additionally, the number of supernodes (i.e. `options.num-supernodes`) must be 20.
