...
def main(cfg: DictConfig):
    ...
    print(f"{cfg.bar.more.blabla = }") # not how when you do access the element directly, it's value gets assigned (not shown as a reference anymore)
    ...
    partial_fn = call(cfg.my_partial_func) #! If you run this with `_partial_` set to false in the config, it will trigger an error since `y` is not defined
    ...
    print(obj.object.x) # it should print now the value we instantiate the object of type `MyClass` with (i.e. =9999)
