...
parser.add_argument("--cid", type=int, required=True, help="Client it. Will be used to assign the i-th partition to the client (default = 0)")
...
def main():
    ...
    assert args.cid < TOTAL_CLIENTS, f"Ensure the specified cid ({args.cid}) is < {TOTAL_CLIENTS}"

    # fabricate dataset for client (using user-suplied cid)
    clients_trainset, clients_valset, _ = prepare_dataset(TOTAL_CLIENTS)

    def client_fn():
        return FlowerClient(args.cid, clients_trainset[args.cid], clients_valset[args.cid])
