...
class SecAggPlusInsAggregator:  # pylint: disable=too-many-instance-attributes
    ...
    def aggregate(self, messages: list[Message]) -> RecordSet:
        ...
        for step in steps:
            ...
            if not res:
                raise ValueError("Secure aggregation halted.")
        agg_rs = RecordSet()
        _set_all_weights(agg_rs, state.aggregated_vector, state.prs_info)
    ...
    def setup_stage(  # pylint: disable=R0912, R0914, R0915
        ...
        content = RecordSet()
        content[RECORD_KEY_CONFIGS] = cfgs_record
...
def _set_all_weights(
    ...
    """Set all weights in the RecordSet."""
    idx = 0
    rs.parameters_records.clear()
    for pr_key in prs_info:
        pr = ParametersRecord()
        for arr_key in prs_info[pr_key][0]:
            pr[arr_key] = Array(all_weights[idx])
            if keep_input:
                idx += 1
            else:
                del all_weights[idx]
        rs.parameters_records[pr_key] = pr
