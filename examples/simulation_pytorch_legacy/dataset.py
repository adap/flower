...
def load(num_partitions: int) -> PartitionedDataset:
    ...
    for xy_train, xy_test in zip(xy_train_partitions, xy_test_partitions):
        ...
        train_dl = DataLoader(
            TensorDataset(torch.tensor(x_train.astype(np.float32)), torch.LongTensor(y_train)),
            batch_size=32,
        )
        test_dl = DataLoader(
            TensorDataset(torch.tensor(x_test.astype(np.float32)), torch.LongTensor(y_test)), batch_size=32
        )
