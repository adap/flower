...
class PlaintextClient(fl.client.Client):
    ...
    def get_parameters(self, config: Dict[str, str]) -> GetParametersRes:
        ...
        try:
            ...
            logging.error(
                f"Client {self.cid}: FULL TRACEBACK:\n{traceback.format_exc()}"
            )
            return GetParametersRes(
    ...
    def fit(self, fit_ins: FitIns) -> FitRes:
        ...
        try:
            ...
            for images, labels in self.trainloader:
                logging.info(
                    f"DEBUG images type: {type(images)}, value: {type(images[0]) if hasattr(images, '__getitem__') else 'N/A'}"
                )
            ...
            updated_params = [
                param.cpu().detach().numpy() for param in self.net.parameters()
            ]
            return FitRes(
                status=Status(Code.OK, "Success"),
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        try:
            ...
            logging.info(
                f"Client {self.cid}: Evaluation loss: {loss:.4f}, accuracy: {accuracy:.4f}"
            )
            return EvaluateRes(
                status=Status(Code.OK, "Success"),
                loss=float(loss),
