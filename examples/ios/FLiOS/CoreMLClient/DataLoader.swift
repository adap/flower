...
let appDirectory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
  .first!
...
class DataLoader {
  ...
  static func trainBatchProvider(
    scenario: Constants.ScenarioTypes, progressHandler: @escaping (Int) -> Void
  ) -> MLBatchProvider {
    return prepareMLBatchProvider(
      filePath: extractTrainFile(scenario: scenario), scenario: scenario,
  ...
  static func testBatchProvider(
    ...
    return prepareMLBatchProvider(
      filePath: extractTestFile(scenario: scenario), scenario: scenario,
      progressHandler: progressHandler)
  }
  ...
  fileprivate static func extractFile(
    from sourceURL: URL, to destinationURL: URL, scenario: Constants.ScenarioTypes
  ) -> String {
  ...
  private static func prepareMLBatchProvider(
    filePath: String, scenario: Constants.ScenarioTypes, progressHandler: @escaping (Int) -> Void
  ) -> MLBatchProvider {
