...
public class MLFlwrClient: Client {
  ...
  public init(
    layerWrappers: [MLLayerWrapper], dataLoader: MLDataLoader, compiledModelUrl: URL, modelUrl: URL
  ) {
    self.modelUrl = modelUrl
    self.parameters = MLParameter(
      layerWrappers: layerWrappers, modelUrl: modelUrl, compiledModelUrl: compiledModelUrl)
  ...
  public func fit(ins: FitIns) -> FitRes {
    let status = Status(code: .ok, message: String())
    let result = runMLTask(
      configuration: parameters.parametersToWeights(parameters: ins.parameters), task: .train)
    let parameters = parameters.weightsToParameters()
  ...
  public func evaluate(ins: EvaluateIns) -> EvaluateRes {
    let status = Status(code: .ok, message: String())
    let result = runMLTask(
      configuration: parameters.parametersToWeights(parameters: ins.parameters), task: .test)
  ...
  private func runMLTask(configuration: MLModelConfiguration, task: MLTask) -> MLResult {
    ...
    let progressHandler: (MLUpdateContext) -> Void = { contextProgress in
      ...
      case .train:
        self.log.info(
          "Epoch \(contextProgress.metrics[.epochIndex] as! Int + 1) finished with loss \(loss)")
      case .test:
    ...
    let completionHandler: (MLUpdateContext) -> Void = { finalContext in
      ...
      let result = MLResult(
        loss: Double(loss)!, numSamples: dataset.count, accuracy: (1.0 - Double(loss)!) * 100)
  ...
  private func saveModel(_ updateContext: MLUpdateContext) {
    ...
    do {
      try fileManager.createDirectory(
        at: tempModelUrl, withIntermediateDirectories: true, attributes: nil)
