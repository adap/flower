...
public class BenchmarkSuite: ObservableObject {
  ...
  public func exportBenchmark() {
    ...
    do {
      let completeBenchmark = try encoder.encode(
        BenchmarkSnaptshot(
          batteryHistory: self.batteryHistory, networkHistory: self.networkHistory,
          actionHistory: self.actionHistory, deviceID: self.deviceID))
