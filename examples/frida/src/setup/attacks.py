...
from .dagmm import DAGMM, get_energy, get_gmm_parameters, train_dagmm, STDDAGMM, train_std_dagmm
import random
...
class YeomAttackEfficient:
    def __init__(self, net, training_datasets, validation_datasets, device, subsets, cfg):
    ...
    def _prepare_canary_tensor(canary_loader, device, cfg):
        """Return all data from a loader as a single tensor pair."""
        xs, ys = [], []
        
        input_key = cfg.text_input if cfg.dataset == "shakespeare" else cfg.image_name
        label_key = cfg.text_label if cfg.dataset == "shakespeare" else cfg.image_label

        for b in canary_loader:
            xs.append(b[input_key]) 
            ys.append(b[label_key])
    ...
    def do_attack(self, parameters, weight_results, threshold=1.0, server_round=None):
        ...
        packed = None
        if getattr(self.cfg, "pack_canary", False):
            packed = _prepare_canary_tensor(canary, self.device, self.cfg)
        ...
        start = time()
        self.net.eval()
        for weights, client_id in weight_results:
            set_parameters_inplace(self.net, weights)
            if packed is None:
                loss, _ = test(self.net, canary, self.device, self.cfg)
            else:
                X, Y = packed
                loss, _ = test_packed(self.net, X, Y)
            canary_losses.append(loss)

        print(f"Total evaluation clients time: {time() - start:.3f} s")

        detection_results, detection_losses = [], []
        for (weights, client_id), loss in zip(weight_results, canary_losses):
            det, det_loss = self._detect(loss, canary_losses, threshold)
            detection_results.append([client_id, det])
            detection_losses.append([client_id, det_loss])
...
class YeomAttack:
    ...
    def do_attack(self, parameters, weight_results, threshold=1.0, server_round=None):
        ...
        if self.cfg.yeom_type == "serverside":
            ...
            train(aux_net, canary, self.device, self.cfg, epochs=self.cfg.server_epochs, verbose=True)

            global_loss, _ = test(aux_net, canary, self.device, self.cfg)
            canary_losses = [self._evaluate_client(weights[0], canary)[0] for weights in weight_results]

            for i, weight_result in enumerate(weight_results):
                detection_result, detection_loss = self._detect_validation(global_loss, canary_losses[i])
            ...
            canary_losses = [self._evaluate_client(weights[0], canary)[0] for weights in weight_results]
            for i, weight_result in enumerate(weight_results):
                detection_result, detection_loss = self._detect(canary_losses[i], canary_losses, threshold)
                detection_losses.append([weight_result[1], detection_loss])
                detection_results.append([weight_result[1], detection_result])
...
def compute_metrics(detection_results, freeriders):
    ...
    false_positives = np.sum(np.logical_and(detection_results, np.logical_not(freeriders)))
    true_negatives = np.sum(np.logical_and(np.logical_not(detection_results), np.logical_not(freeriders)))
    false_negatives = np.sum(np.logical_and(np.logical_not(detection_results), freeriders))
...
class CosineAttackEfficient:
    ...
    def do_attack(self, global_model, weight_results, server_round=None):
        grads_clients = [[w[1], compute_update(global_model, w[0], self.name_layer_grads)] for w in weight_results]
        grads_clients.sort(key=lambda x: int(x[0]))
        if self.cfg.dynamic_canary:
            canary = self.canary[server_round - 1]
        else:
            canary = self.canary
        canary = DataLoader(canary.dataset, batch_size=self.cfg.canary_batch_size, shuffle=True)
        ...
        for client_id in range(len(grads_clients)):
            ...
            validation_subset = DataLoader(validation_subset.dataset, batch_size=self.cfg.canary_batch_size, shuffle=True)
            model_val_grads = self._compute_model_gradients(global_model, validation_subset)

            all_cos_sims = []
            _, grad_client = grads_clients[client_id]
            grad_client = grad_client.to(self.device)

            with torch.no_grad():
                for batch_grads in model_val_grads:         
                    if batch_grads.dtype != grad_client.dtype:
                        grad_client = grad_client.to(dtype=batch_grads.dtype)
                    sims = F.cosine_similarity(
                        batch_grads,
                        grad_client.unsqueeze(0).expand_as(batch_grads),
                        dim=1
                    )
                    all_cos_sims.extend(sims.cpu().tolist())
            distributions_nonmember.append([client_id, torch.tensor(all_cos_sims)])

        print(f"Time dist nonmember: {(time() - time_start_nonmember):.2f} seconds")

        time_start_member = time()
        distributions_member = []
        model_val_grads = self._compute_model_gradients(global_model, canary)
        for client_id in range(len(grads_clients)):
            _, grad_client = grads_clients[client_id]
            grad_client = grad_client.to(self.device)
            ...
            with torch.no_grad():
                for batch_grads in model_val_grads:         
                    ...
                    all_cos_sims.extend(sims.cpu().tolist())            
            distribution_member = torch.tensor(all_cos_sims).cpu()
            distributions_member.append([client_id, distribution_member])

        distributions_nonmember = [x[1] for x in sorted(distributions_nonmember, key=lambda x: x[0])]
        distributions_member = [x[1] for x in sorted(distributions_member,key=lambda x: x[0])]
        ...
        for client_id in range(len(distributions_nonmember)):
            ...
            stat, pvalue = scipy.stats.ttest_ind(distr_nonmember, distr_member, equal_var=False)
            if pvalue < 0.05:
        ...
        return [x[1] for x in detection_results], (distributions_member, distributions_nonmember)

    def _compute_model_gradients(self, model, dataset):
        model_gradients = []
        model.to(self.device)
        total_samples = 0
        
        input_key = self.cfg.text_input if self.cfg.dataset == "shakespeare" else self.cfg.image_name
        label_key = self.cfg.text_label if self.cfg.dataset == "shakespeare" else self.cfg.image_label

        for batch in dataset:
            if total_samples >= self.cfg.subset_samples:
                break
                
            inputs, labels = batch[input_key].to(self.device), batch[label_key].to(self.device)
            batch_size = inputs.shape[0]
            total_samples += batch_size

            batch_grads = self._compute_batch_gradients(model, inputs, labels)
            model_gradients.append(batch_grads)

            del inputs, labels, batch_grads
...
class CosineAttack:
    ...
    def do_attack(self, global_model, weight_results, server_round=None):
        ...
        for client_id in range(len(grads_clients)):
            validation_subset = self.validation_datasets[client_id]
            validation_subset = DataLoader(validation_subset.dataset, batch_size=self.cfg.canary_batch_size, shuffle=True)
            distribution_nonmember = self._attack(global_model, validation_subset, grads_clients[client_id][1])
        ...
        for client_id in range(len(grads_clients)):
            ...
            distribution_member = self._attack(global_model, canary, grads_clients[client_id][1])
            distributions_member.append([client_id, distribution_member])
        ...
        for client_id in range(len(distributions_nonmember)):
            if self.average > 1:
                distr_nonmember = self._average_distributions(self.cosine_nonmembers, client_id)
                distr_member = self._average_distributions(self.cosine_members, client_id)
            else:
                distr_nonmember = distributions_nonmember[client_id]
                distr_member = distributions_member[client_id]
    ...
    def _average_distributions(self, distributions, client_id):
        ...
        tensors_to_average = [distributions[-i][client_id].numpy() for i in range(1, num_epochs + 1)]
...
def zscore_detect(samples, lower_threshold=1, higher_threshold=1):
    ...
    return ([True if z > higher_threshold or -z > lower_threshold else False for z in zscores], zscores)
...
class DAGMMAttack:
    def __init__(self, n_encoder_layers, n_gmm_layers, device, batch_size=16, epochs=100):
    ...
    def do_attack(self, prev_params, weight_results):
        ...
        grads_clients = [
            [w[1], np.concatenate([(new - old).flatten() for old, new in zip(prev_params, w[0])])] for w in weight_results
        ]
        grads_clients.sort(key=lambda x: int(x[0]))
        ...
        for batch in data_loader:
            ...
            energies = [
                get_energy(f, mixture_probabilities, means, covariances).detach().cpu().numpy() for f in combined_features
            ]
...
class STDDAGMMAttack:
    def __init__(self, n_encoder_layers, n_gmm_components, device, batch_size=16, epochs=100):
        self.n_encoder_layers = n_encoder_layers
        self.device = device
        self.batch_size = batch_size
        self.epochs = epochs
    ...
    def do_attack(self, prev_params, weight_results, zscore_threshold):
        ...
        grads_clients = [
            [w[1], np.concatenate([(new - old).flatten() for old, new in zip(prev_params, w[0])])]
            for w in weight_results
        ]
        grads_clients.sort(key=lambda x: int(x[0]))
        grads = torch.tensor(np.array([g[1] for g in grads_clients]), dtype=torch.float32)
...
class CosineSimilarityAttack:
    def do_attack(self, global_model, weight_results, threshold=1.0, server_round=None):
        grads_clients = [[w[1], compute_update(global_model, w[0])] for w in weight_results]
        grads_clients.sort(key=lambda x: int(x[0]))
...
def pia(global_model, prev_params, weight_results, num_examples, cfg, attack_loaders, device):
    return old_pia(
        global_model,
        prev_params,
        weight_results,
...
class DistScoreAttack:
    ...
    def do_attack(self, global_model, prev_params, weight_results, num_examples, cfg, threshold=1.0):
        labels = pia(global_model, prev_params, weight_results, num_examples, cfg, self.attack_loaders, self.device)
    ...
    def detect(self, labels, threshold=1, verbose=True):
        ...
        no_grads_labels_normalised = np.ones_like(global_labels) / np.size(global_labels, axis=0)
        ...
        self.global_labels_normalised = global_labels / np.sum(global_labels)
        if verbose:
            print("detection results distance defense: {}, zscores: {}".format(detection_results, zscores))
...
class InconsistencyAttack:
    ...
    def do_attack(self, global_model, prev_params, weight_results, num_examples, cfg, threshold):
        labels = pia(global_model, prev_params, weight_results, num_examples, cfg, self.attack_loaders, self.device)
    ...
    def detect(self, labels, threshold=1.0, verbose=True):
        ...
        if self.labels_normalised is None:
            ...
            return [False] * np.size(labels, axis=0), ([0] * np.size(labels, axis=0), [0] * np.size(labels, axis=0))
        else:
            self.labels_normalised = np.append(self.labels_normalised, np.expand_dims(labels_normalised, 0), axis=0)
        if np.size(self.labels_normalised, 0) > 2:
            np.delete(self.labels_normalised, 0, axis=0)

        inconsistencies = np.mean(np.linalg.norm(np.diff(self.labels_normalised, axis=0), ord=2, axis=2), axis=0)
        detection_results, zscores = zscore_detect(inconsistencies, threshold, threshold)
        if verbose:
            print("detection results inconsistency defense: {}, zscores: {}".format(detection_results, zscores))
...
def iqr_detect(samples, lower_precentile=25, higher_precentile=75, lower_threshold=1.5, higher_threshold=1.5):
...
def cluster_detect(samples, threshold_ratio=1.5):
    ...
    while max_idx - min_idx + 1 < stop_size:
        if min_idx > 0 and max_idx < np.size(ordered) - 1:
            if ordered[min_idx] - ordered[min_idx - 1] < ordered[max_idx + 1] - ordered[max_idx]:
    ...
    while min_idx > 0 and ordered[min_idx] - ordered[min_idx - 1] <= threshold:
        threshold = np.max((threshold, (ordered[min_idx] - ordered[min_idx - 1]) * threshold_ratio))
        min_idx -= 1
    while max_idx < np.size(ordered) - 1 and ordered[max_idx + 1] - ordered[max_idx] <= threshold:
        threshold = np.max((threshold, (ordered[max_idx + 1] - ordered[max_idx]) * threshold_ratio))
        max_idx += 1
...
def cluster_zscore_detect(samples):
    ...
    while max_idx - min_idx + 1 < stop_size:
        if min_idx > 0 and max_idx < np.size(ordered) - 1:
            ...
            threshold = np.max((ordered[max_idx + 1] - ordered[max_idx], threshold))
            max_idx += 1
    zscores = 2 * (samples - np.mean(ordered[min_idx : max_idx + 1])) / (ordered[max_idx] - ordered[min_idx])
