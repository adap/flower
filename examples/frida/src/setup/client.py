...
def add_gaussian_noise(net, cfg, device):
    for param in net.parameters():
...
class FreeRider(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        if self.freerider_type == "advanced_disguised":
            ...
            if config["prev_params"] is None:
                ...
                theta_t = [
                    torch.tensor(param, device=self.device) for param in parameters
                ]
                theta_t_1 = [
                    torch.tensor(param, device=self.device)
                    for param in config["prev_params"]
                ]
                prenoise_parameters = [
                    2 * new - old for new, old in zip(parameters, config["prev_params"])
                ]
                set_parameters(self.net, prenoise_parameters)
                self.linear_noising(config["server_round"], config["std_noise"])
            ...
            if config["prev_params"] is None or config["prev_prev_params"] is None:
                ...
                theta_t_2 = [
                    torch.tensor(param, device=self.device)
                    for param in config["prev_prev_params"]
                ]

                theta_t_flat = torch.cat([torch.flatten(param) for param in theta_t])
                theta_t_1_flat = torch.cat(
                    [torch.flatten(param) for param in theta_t_1]
                )
                theta_t_2_flat = torch.cat(
                    [torch.flatten(param) for param in theta_t_2]
                )
                ...
                prenoise_parameters = [
                    (new + (new - old) * scale.item())
                    for new, old in zip(parameters, config["prev_params"])
                ]
                set_parameters(self.net, prenoise_parameters)
                prob = 1

                for param, t, t1, t2 in zip(
                    self.net.parameters(), theta_t, theta_t_1, theta_t_2
                ):
                    variance = torch.var(t - t1)
                    if torch.rand(1).item() < prob:
                        std = variance.sqrt().item()
                        noise = torch.normal(
                            mean=0.0, std=std, size=param.size(), device=self.device
                        )
                        param.data += noise
    ...
    def advanced_attack(self, parameters, config):
        theta0 = [
            torch.tensor(param, device=self.device) for param in config["params0"]
        ]
        theta1 = [
            torch.tensor(param, device=self.device) for param in config["params1"]
        ]
        theta_t = [torch.tensor(param, device=self.device) for param in parameters]
        theta_t_1 = [
            torch.tensor(param, device=self.device) for param in config["prev_params"]
        ]
        theta_t_2 = [
            torch.tensor(param, device=self.device)
            for param in config["prev_prev_params"]
        ...
        theta_update = [
            curr + (curr - prev) * scale.item()
            for curr, prev in zip(theta_t, theta_t_1)
        ]
        theta_update_flat = torch.cat([torch.flatten(param) for param in theta_update])
        C = (
            torch.norm(theta_0_flat - theta_t_flat, p=2)
            / torch.norm(theta_update_flat - theta_t_flat, p=2)
        ).item()

        lambda_t = np.log(
            (l_t.item() / l_1.item()) ** (1 / (config["server_round"] - 1))
        )
        expected_cos_beta = (C**2) / (
            C**2 + np.exp(2 * lambda_t * config["server_round"]).item()
        )
        ...
        phi_t = torch.sqrt(torch.tensor(numerator, device=self.device)) * torch.norm(
            theta_t_flat - theta_t_1_flat, p=1
        )
        ...
        for u_f in U_f:
            ...
            noise = torch.normal(
                0, phi_t.item() / l, size=u_f.shape, device=self.device
            )
            noisy_u_f = u_f + mask * noise
            noisy_update.append(noisy_u_f)

        noisy_update = [
            curr + noise_update for curr, noise_update in zip(theta_t, noisy_update)
        ]
    ...
    def linear_noising_random(self, iteration, std_noise, prob):
        for param in self.net.parameters():
            if torch.rand(1).item() < prob:
                std = (
                    self.cfg.multiplicator * std_noise * iteration ** (-self.cfg.power)
                )
                noise = torch.normal(
                    mean=0.0, std=std, size=param.size(), device=self.device
                )
...
class FreeRiderSubset(fl.client.NumPyClient):
    def __init__(
        self, cid, net, trainloader_len, freerider_type, subsetloaders, device, cfg
    ):
        self.freerider_type = freerider_type
        self.net = net.to(device)
        self.cid = cid
...
class ClientSubset(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        return (
            get_parameters(self.net),
            len(self.trainloader) + len(subset),
            {"cid": self.cid},
        )
    ...
    def client_fn(cid: str):
        ...
        if vector_freeriders[cid_int]:
            return FreeRider(
                cid,
                deepcopy(net),
                len(trainloaders[cid_int].sampler),
                cfg.freerider_type,
                device,
                cfg,
            ).to_client()
...
def generate_client_fn_subset(
    net, trainloaders, valloaders, vector_freeriders, subsetloaders, cfg, device
):
    def client_fn(cid: str):
        cid_int = int(cid)
        if vector_freeriders[cid_int]:
            if cfg.freerider_canary:
                return FreeRiderSubset(
                    cid,
                    deepcopy(net),
                    len(trainloaders[cid_int].sampler),
                    cfg.freerider_type,
