...
class STDDAGMM(nn.Module):
    def __init__(
        ...
        for i in range(len(encoder_dims) - 1):
            ...
            if (
                i < len(encoder_dims) - 2
            ):  # Don't add BN and activation after last layer
                self.encoder.append(BatchNorm1d(encoder_dims[i + 1]))
                self.encoder.append(
                    nn.ReLU()
                )  # Using ReLU instead of Tanh for better gradient flow
...
class STDDAGMMLoss(nn.Module):
    ...
    def forward(self, gamma, features, reconstruction_error):
        ...
        energy_loss = torch.mean(
            self._compute_energy(features, mixture_probs, means, covariances)
        )
    ...
    def _compute_energy(self, features, phi, mu, sigma):
        ...
        for i in range(n_components):
            ...
            log_2pi = feature_dim * torch.log(
                torch.tensor(2 * 3.14159, device=features.device)
            )
        ...
        total_energy = torch.where(
            torch.isfinite(total_energy), total_energy, torch.zeros_like(total_energy)
        )
...
class DAGMM(nn.Module):
    def __init__(self, n_encoder_layers, n_gmm_layers, lambda_1=0.1, lambda_2=0.005):
        ...
        self.decoder = self._build_module_list(
            list(reversed(n_encoder_layers[1:])), nn.Tanh()
        )
    ...
    def forward(self, x):
        ...
        combined_features = torch.cat(
            (compressed, self._get_features(x, reconstruction)), dim=1
        )
        error = torch.cdist(
            x.permute(1, 0).unsqueeze(1), reconstruction.permute(1, 0).unsqueeze(1), p=2
        ).squeeze()
    ...
    def _get_features(input, reconstruction):
        euclidean_distance = torch.cdist(
            input.unsqueeze(1), reconstruction.unsqueeze(1), p=2
        ).squeeze()
        return torch.stack(
            (
                torch.std(input, dim=1),
                torch.cosine_similarity(input, reconstruction),
                euclidean_distance,
            ),
            dim=1,
        )
...
class DAGMMLoss(nn.Module):
    ...
    def forward(self, membership_estimations, combined_features, error):
        ...
        diagonal_component = torch.sum(
            torch.reciprocal(torch.diagonal(covariances, dim1=-2, dim2=-1))
        )
        return (
            error_component
            + self.lambda_1 * energy_component
            + self.lambda_2 * diagonal_component
        )
...
def get_energy(sample, mixture_probabilities, means, covariances):
    ...
    term2 = (
        -0.5 * diffs.unsqueeze(1) @ torch.pinverse(covariances) @ diffs.unsqueeze(2)
    ).squeeze()
...
def get_gmm_parameters(membership_estimations, combined_features):
    ...
    covariances = torch.sum(
        membership_estimations.T.unsqueeze(-1).unsqueeze(-1)
        * (diffs.unsqueeze(-1) @ diffs.unsqueeze(-2)),
        dim=1,
