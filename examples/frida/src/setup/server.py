...
def get_evaluate_fn(model, valloader, device, cfg):
    ...
    def evaluate_fn(server_round, parameters, config):
        ...
        if total == 0:
            print(
                "Warning: Evaluation finished with total=0. Valloader might be empty."
            )
...
class PrivacyAttacksForDefense(fl.server.strategy.FedAvg):
        ...
        *,
        ...
        for attack_type in self.attack_types:
            if attack_type == "yeom":
                ...
                attack = CosineAttack(
                    self.net, validation_datasets, device, subsets, cfg
                )

            elif attack_type == "stddagmm":
                cfg.n_encoder_layers.insert(
                    0, sum(p.numel() for p in self.net.parameters() if p.requires_grad)
                )
                attack = STDDAGMMAttack(
                    cfg.n_encoder_layers,
                    cfg.n_gmm_layers,
                    device,
                    cfg.dagmm_batch_size,
                    cfg.dagmm_epochs,
                )
                ...
                attack = DistScoreAttack(
                    ...
                    load_attack_sets_from_noise(
                        input_shape,
                        cfg.num_classes,
                        cfg.sample_per_label,
                        cfg.decaf_batch_size,
                        cfg,
                    ),
                )
    ...
    def configure_fit(
        ...
        if server_round > 1:
            ...
            for tens_A, tens_B in zip(
                parameters_to_ndarrays(parameters), self.prev_parameters
            ):
                sum_abs_diff += torch.sum(
                    torch.abs(torch.tensor(tens_A) - torch.tensor(tens_B))
                )
                abs_diffs.append(torch.abs(torch.tensor(tens_A) - torch.tensor(tens_B)))
            num_params = sum(
                [np.prod(tensor.size()) for tensor in list(self.net.parameters())]
            )
        ...
        if (
            self.cfg.freerider_type == "gradient_noiser"
            or self.cfg.freerider_type == "advanced"
            or self.cfg.freerider_type == "gradient_noiser_old"
        ):
    ...
    def aggregate_fit(
        ...
        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), int(fit_res.metrics["cid"]))
        ...
        num_examples = [
            (fit_res.num_examples, int(fit_res.metrics["cid"]))
            for _, fit_res in results
        ]
        ...
        for attack_type in self.attack_types:
            ...
            if attack_type == "yeom":
                detection_results, detection_metrics = self.attacks[
                    attack_type
                ].do_attack(
                ...
                detection_results, detection_metrics = self.attacks[
                    attack_type
                ].do_attack(self.net, weights_results, server_round)
            elif attack_type == "dagmm":
                detection_results, detection_metrics = self.attacks[
                    attack_type
                ].do_attack(
                ...
                attack_type == "l2"
                or attack_type == "std"
                or attack_type == "cosine_similarity"
            ):
                detection_results, detection_metrics = self.attacks[
                    attack_type
                ].do_attack(self.net, weights_results, self.cfg.zscore_threshold)
            elif attack_type in ("inconsistency", "dist_score"):
                detection_results, detection_metrics, pia_results = self.attacks[
                    attack_type
                ].do_attack(
                    self.net,
            ...
            print(
                f"  {attack_type.upper()} attack execution time: {individual_attack_times[attack_type]:.3f} seconds"
            )
        ...
        if self.cfg.mitigation:
            parameters_aggregated = ndarrays_to_parameters(
                aggregate(
                    [
                        weight_result
                        for (detection, weight_result) in zip(
                            detection_results, weights_results
                        )
                        if not detection
                    ]
                )
        ...
        # Calculate metrics
        freerider_metrics = self.attacks[self.attack_types[0]].get_metrics(
            detection_results, self.freeriders
        )
        ...
        if "yeom" in self.attack_types:
            freerider_metrics["detection_loss"] = attack_results["yeom"][
                "detection_metrics"
            ]

        if "cosine" in self.attack_types:
            freerider_metrics["cosine_metrics"] = attack_results["cosine"][
                "detection_metrics"
            ]
        ...
        if "std" in self.attack_types:
            freerider_metrics["std_metrics"] = attack_results["std"][
                "detection_metrics"
            ]

        if "cosine_similarity" in self.attack_types:
            freerider_metrics["cosim_metrics"] = attack_results["cosine_similarity"][
                "detection_metrics"
            ]

        if any(k in self.attack_types for k in ("dist_score", "inconsistency")):
            freerider_metrics["pia"] = (
                ("dist_score" in self.attack_types)
                and (attack_results.get("dist_score") or {}).get("pia_results")
            ) or (
                ("inconsistency" in self.attack_types)
                and (attack_results.get("inconsistency") or {}).get("pia_results")
            )

        freerider_metrics["historical_detection_results"] = (
            self.history_detection_results
        )
    ...
    def print_cumulative_stats(self, server_round):
        ...
        print(
            f"Average attack time per round: {self.total_attack_time/server_round:.3f} seconds"
        )
        print(f"Total aggregation time: {self.total_aggregation_time:.2f} seconds")
        print(
            f"Average aggregation time per round: {self.total_aggregation_time/server_round:.3f} seconds"
        )

        if self.round_metrics:
            avg_memory = sum(m["memory_usage_mb"] for m in self.round_metrics) / len(
                self.round_metrics
            )
...
class PrivacyAttacksForDefenseFedProx(PrivacyAttacksForDefense):
    ...
    def configure_fit(
        ...
        print(f"\n{'='*60}")
        print(
            f"FL ROUND {server_round} - CONFIGURATION PHASE (FedProx μ={self.proximal_mu})"
        )
        print(f"{'='*60}")
        print(f"Number of clients selected: {len(clients)}")
        print(f"Configuration time: {config_time:.3f} seconds")
