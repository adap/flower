...
def train(net, trainloader, device, cfg, epochs=1, proximal_mu=0.0, global_params=None):
    ...
    if proximal_mu > 0.0 and global_params is not None:
        ...
        params_global_flat = torch.cat([torch.flatten(param) for param in global_params])
    ...
    for _ in range(epochs):
        for batch in trainloader:
            if cfg.dataset == "shakespeare":
                inputs, labels = batch[cfg.text_input].to(device), batch[cfg.text_label].to(device)
            else:
                inputs, labels = batch[cfg.image_name].to(device), batch[cfg.image_label].to(device)
            ...
            if proximal_mu > 0.0 and global_params is not None:
                ...
                params_localround_flat = torch.cat([torch.flatten(param) for param in params_localround])
                proximal_term = torch.norm(params_localround_flat - params_global_flat, p=2)**2
