...
def train_with_opacus(
        pipe, dataloader, device, model_dtype,
    ...
    for _ in range(epochs):
        for batch_idx, batch in enumerate(dataloader):
            if isinstance(batch["pixel_values"], list):
                images = torch.stack([batch["pixel_values"][0]])
            else:
                images = batch["pixel_values"][:1]  # Single sample
            ...
            # Clean memory
            del images, latents, noise, noisy_latents, text_embeddings, noise_pred
    ...
    print(f"Average loss: {avg_loss:.6f} and epsilon value: {epsilon}")
...
def add_gaussian_noise(lora_state, sigma, max_norm):
    return {
        k: v + torch.randn_like(v) * sigma * max_norm
        for k, v in lora_state.items()
    }

def add_laplace_noise(lora_state, epsilon, max_norm):
    scale = max_norm / epsilon
    return {
        k: v + torch.distributions.Laplace(
            loc=0.0, scale=scale
        ).sample(v.shape).to(v.device)
        for k, v in lora_state.items()
    }
