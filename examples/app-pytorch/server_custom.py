...
def main(driver: Driver, context: Context):
    ...
    # -------------------------------------------------------------------------- Driver SDK
    grpc_driver, run_id = driver._get_grpc_driver_and_run_id()
    grpc_driver.connect()
    create_run_res: driver_pb2.CreateRunResponse = grpc_driver.create_run(
    ...
    for server_round in range(num_rounds):
        ...
        if anonymous_client_nodes:
            ...
            while True:
                ...
                # ---------------------------------------------------------------------- Driver SDK
                get_nodes_res: driver_pb2.GetNodesResponse = grpc_driver.get_nodes(
                    req=get_nodes_req
                )
                # ---------------------------------------------------------------------- Driver SDK
        ...
        server_message_proto: transport_pb2.ServerMessage = (
            serde.server_message_to_proto(server_message=ServerMessage(fit_ins=fit_ins))
        )
        ...
        # ---------------------------------------------------------------------- Driver SDK
        push_task_ins_res: driver_pb2.PushTaskInsResponse = grpc_driver.push_task_ins(
        ...
        while True:
            ...
            pull_task_res_res: driver_pb2.PullTaskResResponse = (
                grpc_driver.pull_task_res(req=pull_task_res_req)
            )
            # ------------------------------------------------------------------ Driver SDK
