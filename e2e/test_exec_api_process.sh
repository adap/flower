...
combined_args="$server_arg $server_auth $exec_api_arg --executor flwr.superexec.deployment:executor --isolation process"

timeout 2m flower-superlink $combined_args --executor-config "$executor_config" &
sl_pid=$!
sleep 2

timeout 2m flwr-serverapp --superlink 127.0.0.1:9091 $client_arg 2>&1 | tee flwr_output.log &
sa_pid=$(pgrep -f "flwr-serverapp")
sleep 2
...
timeout 2m flower-supernode ./ $client_arg \
    ...
    $server_arg --supernode "0.0.0.0:9094" --isolation process &
sn1_pid=$!
sleep 2

timeout 2m flwr-clientapp --supernode "127.0.0.1:9094" $client_arg &
ca1_pid=$!
sleep 2
...
timeout 2m flower-supernode ./ $client_arg \
    ...
    $server_arg --supernode "0.0.0.0:9095" --isolation process &
sn2_pid=$!
sleep 2

timeout 2m flwr-clientapp --supernode "127.0.0.1:9095" $client_arg &
ca2_pid=$!
sleep 2
...
while [ "$found_success" = false ] && [ $elapsed -lt $timeout ]; do
    if grep -q "Run finished" flwr_output.log; then
        ...
        found_success=true
        kill $ca1_pid; kill $ca2_pid;
        sleep 1; kill $sa_pid; 
        sleep 1; kill $sn1_pid; kill $sn2_pid;
        sleep 1; kill $sl_pid;
    else
...
if [ "$found_success" = false ]; then
    ...
    kill $sa_pid; kill $ca1_pid; kill $ca2_pid;
    sleep 1; kill $sn1_pid; kill $sn2_pid;
    sleep 1; kill $sl_pid;
fi
