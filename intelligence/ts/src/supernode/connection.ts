...
export async function grpcRequestResponse(
  ...
  async function getRun(runId: bigint): Promise<Run> {
    ...
    return {
      ...
      overrideConfig: res.response.run?.overrideConfig
        ? userConfigFromProto(res.response.run?.overrideConfig)
        : {},
    } as Run;
  }
  ...
  async function getFab(fabHash: string): Promise<Fab> {
    ...
    return {
      hashStr: res.response.fab?.hashStr,
      content: res.response.fab?.content,
    } as Fab;
  }
