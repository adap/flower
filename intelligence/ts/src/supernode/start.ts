...
export async function startClientInternal(
  ...
  function onSuccess(retryState: RetryState): void {
    ...
    if (retryState.tries > 1) {
      console.log(
        `Connection successful after ${retryState.elapsedTime} seconds and ${retryState.tries} tries.`
      );
    }
  }
  ...
  function onGiveup(retryState: RetryState): void {
    if (retryState.tries > 1) {
      console.warn(
        `Giving up reconnection after ${retryState.elapsedTime} seconds and ${retryState.tries} tries.`
      );
    }
  }
  ...
  const address = parsedAdress.version
    ? `[${parsedAdress.host}]:${parsedAdress.port}`
    : `${parsedAdress.host}:${parsedAdress.port}`;
  ...
  while (!appStateTracker.interrupt) {
    ...
    while (!appStateTracker.interrupt) {
      try {
        ...
        console.log(
          `Received: ${message.metadata.messageType} message ${message.metadata.messageId}]`
        );
        ...
        let replyMessage = message.createErrorReply({
