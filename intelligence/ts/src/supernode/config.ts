...
export function loadAndValidate(
   ...
   */
  const configPath = providedPath
    ? path.resolve(providedPath.toString())
    : path.join(process.cwd(), 'pyproject.toml');
...
export function validateFields(config: AnyDict): [boolean, string[], string[]] {
  ...
  if (!config['tool'] || !config['tool']['flwr'] || !config['tool']['flwr']['app']) {
...
export function flattenDict(
  rawDict: { [key: string]: any } | undefined,
  parentKey = ''
): UserConfig {
  if (!rawDict) {
    return {};
...
export function unflattenDict(flatDict: { [key: string]: any }): {
  [key: string]: any;
} {
