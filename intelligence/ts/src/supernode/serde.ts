...
export const parametersToProto = (params: LocalParams): ProtoParams => {
  return {
    ...
    tensorType: params.tensorType,
  } as ProtoParams;
};

export const parametersFromProto = (protoParams: ProtoParams): LocalParams => {
  return {
    tensors: protoParams.tensors,
    tensorType: protoParams.tensorType,
  } as LocalParams;
};
...
function recordValueToProto(value: any): ProtoMetricsRecordValue | ProtoConfigsRecordValue {
  if (typeof value === 'number') {
    ...
    if (typeof value[0] === 'number') {
      return {
        value: { oneofKind: 'doubleList', doubleList: { vals: value } },
      };
      ...
      return {
        value: { oneofKind: 'stringList', stringList: { vals: value } },
      };
...
export const messageFromTaskIns = (taskIns: TaskIns): Message => {
  ...
  let message = new Message(
    ...
    taskIns.task?.error
      ? ({
          code: Number(taskIns.task.error.code),
          reason: taskIns.task.error.reason,
        } as LocalError)
      : null
  );
...
export const messageToTaskRes = (message: Message): TaskRes => {
  ...
  (taskRes.taskId = ''), (taskRes.groupId = md.groupId);
  ...
  task.error =
    message.error === null
      ? undefined
      : ({
          code: BigInt(message.error.code),
...
export const userConfigValueToProto = (userConfigValue: UserConfigValue): Scalar => {
  switch (typeof userConfigValue) {
    case 'string':
      return {
        scalar: { oneofKind: 'string', string: userConfigValue },
      } as Scalar;
    case 'number':
      return {
        scalar: { oneofKind: 'double', double: userConfigValue },
      } as Scalar;
    case 'bigint':
      return {
        scalar: { oneofKind: 'sint64', sint64: userConfigValue },
      } as Scalar;
    case 'boolean':
