...
class MlxEngine: Engine {
        ...
        case .idle:
            ...
            let modelContainer = try await LLMModelFactory.shared.loadContainer(
                ...
                if let progressHandler = progressHandler {
                    let ret = Progress(totalBytes: Int(progress.totalUnitCount), loadedBytes: Int(progress.completedUnitCount), percentage: progress.fractionCompleted, description: modelConfiguration.name)
                    progressHandler(ret)
                }
            }
    ...
    func fetchModel(model: String, callback: @escaping (Progress) -> Void) async throws -> Void {
        modelConfiguration = modelMapping[model] ?? ModelRegistry.llama3_2_1B_4bit
