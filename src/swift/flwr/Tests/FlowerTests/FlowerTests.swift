...
final class FlowerTests: XCTestCase {
  ...
  let metrics: Metrics = [
    "MAE": Scalar(float: 0.013), "MSE": Scalar(float: 0.03), "REC": Scalar(bool: true),
  ]
  let properties: Properties = [
    "MSE": Scalar(str: "test"), "Property1": Scalar(str: "test"), "REC": Scalar(int: 4, str: "abc"),
  ]
  ...
  func testEvaluateInsProtoConversion() throws {
    ...
    let evaluateIns = EvaluateIns(
      parameters: parameters, config: ["config1": scalar1, "config2": scalar2])
