...
func fitResFromProto(msg: Flwr_Proto_ClientMessage.FitRes) -> FitRes {
  ...
  let status = statusFromProto(msg: msg.status)
  return FitRes(
    parameters: parameters, numExamples: Int(msg.numExamples), metrics: metrics, status: status)
}
...
func evaluateResFromProto(msg: Flwr_Proto_ClientMessage.EvaluateRes) -> EvaluateRes {
  let status = statusFromProto(msg: msg.status)
  return EvaluateRes(
    loss: msg.loss, numExamples: Int(msg.numExamples),
    metrics: metricsFromProto(proto: msg.metrics), status: status)
}
