...
@available(iOS 14.0, *)
class FlowerClientInterceptors: ClientInterceptor<
...
>
{
  ...
  override func receive(
    _ part: GRPCClientResponsePart<Flwr_Proto_ServerMessage>,
    context: ClientInterceptorContext<Flwr_Proto_ClientMessage, Flwr_Proto_ServerMessage>
  ) {
    ...
    case let .message(response):
      ...
      log.info(
        "< Received response '\(decipherServerMessage(response.msg!))' with text size '\(msg.count)'"
      )
  ...
  override func send(
    _ part: GRPCClientRequestPart<Flwr_Proto_ClientMessage>, promise: EventLoopPromise<Void>?,
    context: ClientInterceptorContext<Flwr_Proto_ClientMessage, Flwr_Proto_ServerMessage>
  ) {
    ...
    case let .message(request, _):
      ...
      log.info(
        "> Sending request \(decipherClientMessage(request.msg!)) with text size \(msg.count)")
