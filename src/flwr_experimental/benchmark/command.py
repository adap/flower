...
def start_server(log_host: str, benchmark: str, setting: str) -> str:
    ...
    return (
        "screen -d -m"
        + f" python3.7 -m flwr_experimental.benchmark.{benchmark}.server"
        + f" --log_host={log_host}"
        + f" --setting={setting}"
    )
...
def start_client(
    ...
    return (
        "screen -d -m"
        + f" python3.7 -m flwr_experimental.benchmark.{benchmark}.client"
...
def download_dataset(benchmark: str) -> str:
    ...
    return f"python3.7 -m flwr_experimental.benchmark.{benchmark}.download"
