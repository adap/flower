...
class RecordSet(TypedDict[str, RecordType]):
    """RecordSet stores groups of arrays, metrics and configs.

    A :class:`RecordSet` is the unified mechanism by which arrays,
    ...
    array_records : Optional[Dict[str, ArrayRecord]]
        A dictionary of :code:`ArrayRecord` that can be used to record
    ...
    >>>  from flwr.common import ConfigsRecord, MetricsRecord, ArrayRecord
    >>>
    ...
    >>>
    >>>  # Adding it to the record_set would look like this
    >>>  my_recordset["my_arrays"] = arr_record
