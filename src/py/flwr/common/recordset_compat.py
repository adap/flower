...
def _recordset_to_fit_or_evaluate_ins_components(
    ...
    parameters_record = recordset.parameters_dict[f"{ins_str}.parameters"]
    ...
    # get config dict
    config_record = recordset.configs_dict[f"{ins_str}.config"]
...
def _fit_or_evaluate_ins_to_recordset(
    ...
    recordset.parameters_dict[f"{ins_str}.parameters"] = parameters_to_parametersrecord(
        ins.parameters, keep_input=keep_input
    )

    recordset.configs_dict[f"{ins_str}.config"] = ConfigsRecord(
...
def _embed_status_into_recordset(
    ...
    recordset.configs_dict[f"{res_str}.status"] = ConfigsRecord(status_dict)
    return recordset


def _extract_status_from_recordset(res_str: str, recordset: RecordSet) -> Status:
    status = recordset.configs_dict[f"{res_str}.status"]
...
def recordset_to_fitres(recordset: RecordSet, keep_input: bool) -> FitRes:
    ...
    ins_str = "fitres"
    parameters = parametersrecord_to_parameters(
        recordset.parameters_dict[f"{ins_str}.parameters"], keep_input=keep_input
    )

    num_examples = cast(
        int, recordset.metrics_dict[f"{ins_str}.num_examples"]["num_examples"]
    )
    configs_record = recordset.configs_dict[f"{ins_str}.metrics"]
    # pylint: disable-next=protected-access
...
def fitres_to_recordset(fitres: FitRes, keep_input: bool) -> RecordSet:
    ...
    recordset.configs_dict[f"{res_str}.metrics"] = ConfigsRecord(
        fitres.metrics  # type: ignore
    )
    recordset.metrics_dict[f"{res_str}.num_examples"] = MetricsRecord(
        {"num_examples": fitres.num_examples}
    )
    recordset.parameters_dict[f"{res_str}.parameters"] = parameters_to_parametersrecord(
        fitres.parameters, keep_input
    )
...
def recordset_to_evaluateres(recordset: RecordSet) -> EvaluateRes:
    ...
    loss = cast(int, recordset.metrics_dict[f"{ins_str}.loss"]["loss"])
...
def evaluateres_to_recordset(evaluateres: EvaluateRes) -> RecordSet:
    ...
    res_str = "evaluateres"
    # loss
    recordset.metrics_dict[f"{res_str}.loss"] = MetricsRecord(
...
def recordset_to_getparametersins(recordset: RecordSet) -> GetParametersIns:
    ...
    config_record = recordset.configs_dict["getparametersins.config"]
    # pylint: disable-next=protected-access
    config_dict = _check_mapping_from_recordscalartype_to_scalar(config_record._data)
...
def getparametersins_to_recordset(getparameters_ins: GetParametersIns) -> RecordSet:
    ...
    recordset.configs_dict["getparametersins.config"] = ConfigsRecord(
...
def getparametersres_to_recordset(
    ...
    recordset.parameters_dict[f"{res_str}.parameters"] = parameters_record
...
def recordset_to_getparametersres(
    ...
    res_str = "getparametersres"
    parameters = parametersrecord_to_parameters(
        recordset.parameters_dict[f"{res_str}.parameters"], keep_input=keep_input
    )
...
def recordset_to_getpropertiesins(recordset: RecordSet) -> GetPropertiesIns:
    ...
    config_record = recordset.configs_dict["getpropertiesins.config"]
    # pylint: disable-next=protected-access
    config_dict = _check_mapping_from_recordscalartype_to_scalar(config_record._data)
...
def getpropertiesins_to_recordset(getpropertiesins: GetPropertiesIns) -> RecordSet:
    ...
    recordset = RecordSet()
    recordset.configs_dict["getpropertiesins.config"] = ConfigsRecord(
...
def recordset_to_getpropertiesres(recordset: RecordSet) -> GetPropertiesRes:
    ...
    config_record = recordset.configs_dict[f"{res_str}.properties"]
    # pylint: disable-next=protected-access
...
def getpropertiesres_to_recordset(getpropertiesres: GetPropertiesRes) -> RecordSet:
    ...
    recordset = RecordSet()
    res_str = "getpropertiesres"
    recordset.configs_dict[f"{res_str}.properties"] = ConfigsRecord(
