...
def test_cid_consistency_all_submit_first_workload_consistency() -> None:
    ...
    for prox in proxies:
        # Register state
        prox.proxy_state.register_workloadstate(workload_id=w_id)
        # Retrieve state
        state = prox.proxy_state.retrieve_workloadstate(workload_id=w_id)
    ...
    for prox in proxies:
        ...
        prox.proxy_state.update_workloadstate(w_id, workload_state=updated_state)
        res = cast(GetPropertiesRes, res)
        assert int(prox.cid) * pi == res.properties["result"]
        assert (
            str(int(prox.cid) * pi)
            == prox.proxy_state.retrieve_workloadstate(w_id).state["result"]
        )
