...
class History:
    ...
    def add_loss_distributed(self, fl_round: int, loss: float) -> None:
        """Add one loss entry (from distributed evaluation)."""
        self.losses_distributed.append((fl_round, loss))

    def add_loss_centralized(self, fl_round: int, loss: float) -> None:
    ...
    def add_metrics_centralized(
        ...
        for key in metrics:
            ...
            self.metrics_centralized[key].append((fl_round, metrics[key]))
