...
class ControlServicer(control_pb2_grpc.ControlServicer):
    ...
    def CreateRun(
        ...
        """Create run ID."""
        log(DEBUG, "ControlServicer.CreateRun")
    ...
    def GetRunStatus(self, request: GetRunStatusRequest, context: grpc.ServicerContext) -> GetRunStatusResponse:
    ...
    def UpdateRunStatus(self, request: UpdateRunStatusRequest, context: grpc.ServicerContext) -> UpdateRunStatusResponse:
        ...
        status_proto = request.status
        new_status = RunStatus(status_proto.phase, status_proto.result, status_proto.reason)
        if new_status.phase != "finished":
            raise NotImplementedError("ControlServicer.UpdateRunStatus can only be used to signal the completion of a run.")
        
        # Check the current status
        status_dict = state.get_run_status({run_id})
        if not status_dict:
            raise ValueError("`run_id` is invalid")
        current_status = status_dict[run_id]
        
        # Check if both the new status and current status is of phase "finished"
        if new_status.phase 
