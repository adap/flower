...
def validate_task_ins_or_res(tasks_ins_res: Union[TaskIns, TaskRes]) -> List[str]:
    ...
    if isinstance(tasks_ins_res, TaskIns):
        ...
        if sum(has_set_fields.values()) == 0:
            validation_errors.append(
                "`task` in `TaskIns` must set at least one of fields {%s}"
                % ", ".join([f"`{o}`" for o in content_fields])
            )
        if has_set_fields[
    ...
    if isinstance(tasks_ins_res, TaskRes):
        ...
        if sum(has_set_fields.values()) == 0:
            validation_errors.append(
                "`task` in `TaskRes` must set at least one of fields {%s}"
                % ", ".join([f"`{o}`" for o in content_fields])
            )
        if has_set_fields[
