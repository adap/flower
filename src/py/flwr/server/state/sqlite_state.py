...
class SqliteState(State):
    ...
    def initialize(self) -> List[Tuple[str]]:
        ...
        self.conn.set_trace_callback(lambda wtf: print(wtf))
    ...
    def _query(
        ...
        query: str,
        data: Optional[
            Union[List[Union[Tuple[Any], Dict[str, Any]]], Tuple[Any]]
        ] = None,
    ) -> List[Tuple[any]]:
    ...
    def get_task_ins(
        ...
        query += ";"
        data = {"node_id": node_id, "limit": limit}
    ...
    def delete_tasks(self, task_ids: Set[UUID]) -> None:
        ...
        with self.conn:
            self.conn.execute(query_1, {"task_id": str(ids[0])})
            self.conn.execute(query_2, {"task_id": str(ids[0])})
