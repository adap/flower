...
# ==============================================================================
import galois
from flwr.common.secure_aggregation.lightsecagg_utils import encode_mask, compute_aggregated_encoded_mask, mask_weights
import numpy as np
from flwr.common.parameter import parameters_to_ndarrays, ndarrays_to_parameters
from flwr.common.typing import Parameters
from flwr.common.secure_aggregation.quantization import quantize
from flwr.common.secure_aggregation.crypto.symmetric_encryption import encrypt, decrypt, generate_shared_key, generate_key_pairs, \
    public_key_to_bytes, bytes_to_public_key
from flwr.common.secure_aggregation.ndarrays_arithmetic import parameters_multiply, factor_combine, get_zero_parameters
...
def ask_encrypted_encoded_masks(client, public_keys_dict: Dict[int, bytes]):
    ...
    encoded_msk_set = encode_mask(client.d, client.N, client.U, client.T, client.GF, client.msk)
...
def ask_masked_models(client, packets, fit_ins):
    ...
    quantized_weights = parameters_multiply(
        quantized_weights, weights_factor)
    quantized_weights = factor_combine(
...
def ask_aggregated_encoded_masks(client, active_clients):
    agg_msk = compute_aggregated_encoded_mask(client.encoded_mask_dict, client.GF, active_clients)
