...
def _increment_context_counter(context: Context) -> None:
    # Read from context
    current_counter: int = context.state.metrics_dict[METRIC][COUNTER]  # type: ignore
    # update and override context
    current_counter += 1
    context.state.metrics_dict[METRIC] = MetricsRecord({COUNTER: current_counter})
...
def make_mock_mod(name: str, footprint: List[str]) -> Mod:
    ...
    def mod(message: Message, context: Context, app: ClientAppCallable) -> Message:
        ...
        message.content.configs_dict[name] = ConfigsRecord()
        ...
        out_message.content.configs_dict[name] = ConfigsRecord()
        return out_message
...
class TestMakeApp(unittest.TestCase):
    ...
    def test_multiple_mods(self) -> None:
        ...
        state = RecordSet()
        state.metrics_dict[METRIC] = MetricsRecord({COUNTER: 0.0})
        ...
        self.assertEqual("".join(message.content.configs_dict.keys()), "".join(trace))
        self.assertEqual(
            "".join(out_message.content.configs_dict.keys()), "".join(reversed(trace))
        )
        self.assertEqual(state.metrics_dict[METRIC][COUNTER], 2 * len(mock_mods))
    ...
    def test_filter(self) -> None:
        ...
        def filter_mod(
            ...
            message.content.configs_dict["filter"] = ConfigsRecord()
            out_message = Message(metadata=message.metadata, content=RecordSet())
            out_message.content.configs_dict["filter"] = ConfigsRecord()
            # Skip calling app
            return out_message
        ...
        # pylint: disable-next=no-member
        self.assertEqual(list(message.content.configs_dict.keys())[0], "filter")
        self.assertEqual(list(out_message.content.configs_dict.keys())[0], "filter")
