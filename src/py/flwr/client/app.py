...
def _wrap_numpy_client(client: NumPyClient) -> Client:
    # Define wrapper class methods

    def constructor(self: Client, numpy_client: NumPyClient) -> None:
        self.numpy_client = numpy_client  # type: ignore

    def get_properties(self: Client, ins: GetPropertiesIns) -> GetPropertiesRes:
    ...
    def get_parameters(self: Client, ins: GetParametersIns) -> GetParametersRes:
    ...
    def fit(self: Client, ins: FitIns) -> FitRes:
    ...
    def evaluate(self: Client, ins: EvaluateIns) -> EvaluateRes:
    ...
    # Dynamically add wrapper type methods (if overridden by )

    if numpyclient_has_get_properties(client=client):
        member_dict["get_properties"] = get_properties

    if numpyclient_has_get_parameters(client=client):
        member_dict["get_parameters"] = get_parameters

    if numpyclient_has_fit(client=client):
        member_dict["fit"] = fit

    if numpyclient_has_evaluate(client=client):
        member_dict["evaluate"] = evaluate

    DynNumPyClientWrapper = type("DynNumPyClientWrapper", (Client,), member_dict)

    # Return instance of newly created class
    return DynNumPyClientWrapper(numpy_client=client)  # type: ignore
