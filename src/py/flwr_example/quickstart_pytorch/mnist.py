...
def test(
    ...
    print(
        "\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n".format(
            test_loss, correct, num_test_samples, 100.0 * correct / num_test_samples
        )
    )
...
class PytorchMNISTClient(fl.client.Client):
    ...
    def fit(self, ins: fl.common.FitIns) -> fl.common.FitRes:
        ...
        # Train model
        num_examples_train : int = train(self.model, self.train_loader, epochs=self.epochs, device=self.device)
    ...
    def evaluate(self, ins: fl.common.EvaluateIns) -> fl.common.EvaluateRes:
        ...
        num_examples_test, loss, accuracy, = test(self.model, self.test_loader, device=self.device)
