...
def get_p_layer_updates(
        p: float, layer_updates: List[np.ndarray],
    ...
    layer_updates_p = [
        layer_update for p_max, layer_update in zip(
            p_max_s, layer_updates) if p_max >= p]
    num_examples_p = sum([
        n for p_max, n in zip(p_max_s, num_examples) if p_max >= p])
    return layer_updates_p, num_examples_p


def fjord_average(
        i: int, layer_updates: List[np.ndarray],
        num_examples: List[int], p_max_s: List[float],
        ...
        for p in p_s[::-1]:
            ...
            if len(layer_updates_p) == 0:
                ...
                if len(update.shape) == 1:
                    ...
                    layer_updates_p = [
                        layer_update[:out_dim] for layer_update
                        in layer_updates_p]
                    update[:out_dim] = reduce(np.add, layer_updates_p
                                              ) / num_examples_p
                else:
                    # weight
                    layer_updates_p = [
                        layer_update[:out_dim, :in_dim] for layer_update
                        in layer_updates_p]
                    update[:out_dim, :in_dim] = reduce(np.add, layer_updates_p
                                                       ) / num_examples_p
        return update
    else:
...
def aggregate(results: List[Tuple[NDArrays, int, float, FJORD_CONFIG_TYPE]],
    ...
    weights_prime: NDArrays = [
        fjord_average(
            i, layer_updates, num_examples,
            p_max_s, p_s, fjord_config, original_parameters)
    ...
    def aggregate_fit(
        ...
        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), fit_res.num_examples,
             fit_res.metrics['max_p'], fit_res.metrics['p_s'],
        ...
        training_losses_str = ", ".join([str(fit_res.metrics['loss'])
                                         for _, fit_res in results])
