...
class pFedHN(FedAvg):
    ...
    def initialize_parameters(self, client_manager: ClientManager) -> Optional[Parameters]:
        ...
        self.hnet = CNNHyper(n_nodes=self.cfg.client.num_nodes, embedding_dim=int(1 + self.cfg.client.num_nodes / 4), in_channels=self.cfg.model.in_channels, n_kernels=self.cfg.model.n_kernels,out_dim=self.cfg.model.out_dim, hidden_dim=100 ,n_hidden=1)
    ...
    def configure_fit(self,server_round:int,parameters:Parameters,client_manager:ClientManager) -> List[Tuple[ClientProxy,FitIns]]:
        ...
        def weights_to_clients(client_id):
            weights = self.hnet(torch.tensor([client_id],dtype=torch.long).to(torch.device("cpu")))
            return weights
    ...
    def gradient_upgradation(self,delta_theta_param:Parameters):
        ...
        param_dict = zip(self.hnet.state_dict().keys(),fl.common.parameters_to_ndarrays(delta_theta_param))
