...
def _load_client_data(context):
    ...
    if _client_data is None:
        ...
        if str(client_id) not in [str(k) for k in ds_dict["client2data"].keys()]:
            print(f"❌ Client ID {client_id} not found! Available: {list(ds_dict['client2data'].keys())}")
        ...
        print(f"📊 Client {client_id} loaded {len(_client_data['train_data'])} training samples")
...
def train(msg: Message, context: Context):
    ...
    trainloader = DataLoader(train_data, batch_size=cfg.data_dist.batch_size, shuffle=True, collate_fn=DefaultDataCollator())
    
    local_epochs = context.run_config["local-epochs"]

    # Determine model type for training using architecture detection
    model_type = "transformer" if cfg.data_dist.model_architecture == "transformer" else "cnn"
...
def evaluate(msg: Message, context: Context):
    ...
    # Convert to DataLoader with DefaultDataCollator for proper tensor batching
    from torch.utils.data import DataLoader
    valloader = DataLoader(test_data, batch_size=cfg.data_dist.batch_size, shuffle=False, collate_fn=DefaultDataCollator())
    ...
    metrics = {
        ...
        # Flower MetricRecord only accepts numeric values (int, float, list[int], list[float])
    }
    metric_record = MetricRecord(metrics)
