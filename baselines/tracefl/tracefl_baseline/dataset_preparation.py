...
This module provides dataset preparation functionality for TraceFL baseline,
supporting multiple datasets and partitioning strategies without caching.
"""
...
def _initialize_medical_dataset(cfg, dat_partitioner_func, fetch_only_test_data):
    """
    Initialize and process a medical dataset (MedMNIST) by applying TraceFL's
    RGB-compatible preprocessing pipeline.
    ...
    train_transformed = train_hf.map(
        apply_medical_transform, batched=True, batch_size=256, num_proc=1).with_format("torch")
    ...
    return {
        ...
        'client2class': {cid: dict(label_counts) for cid, label_counts in client2class.items()},
...
def _initialize_text_dataset(cfg, dat_partitioner_func, fetch_only_test_data):
    ...
    train_transformed = train_dataset.map(
        apply_text_transform, batched=True, batch_size=256, num_proc=1).with_format("torch")
...
def _initialize_image_dataset(cfg, dat_partitioner_func, fetch_only_test_data):
    ...
    d['client2data'] = {k: v.map(
        transforms['train'], batched=True, batch_size=256, num_proc=8).with_format("torch") 
...
def get_labels_count(partition, target_label_col):
    ...
    """
    label2count = Counter(_label_to_str(example[target_label_col])  # type: ignore
...
def _fix_partition(cfg, c_partition, target_label_col):
    ...
    partition_labels_count = {str(label): int(count)
                              for label, count in partition_labels_count_raw.items()}
    return {'partition': ds, 'partition_labels_count': partition_labels_count}


def _partition_helper(partitioner, cfg, target_label_col, fetch_only_test_data, subtask, train_dataset=None, test_dataset=None):
    """
    ...
    if train_dataset is not None and test_dataset is not None:
        ...
        fds = FederatedDataset(dataset=dataset_dict, partitioners={"train": partitioner})
    else:
    ...
    for cid in range(cfg.num_clients):
        ...
        if cfg.max_per_client_data_size > 0:
            ...
            logging.info(f' No data partition fix required for client {cid}')
            temp_dict = {'partition': client_partition, 'partition_labels_count': get_labels_count(
...
def _dirichlet_data_distribution(cfg, target_label_col, fetch_only_test_data, train_dataset=None, test_dataset=None, subtask=None):
    """
    ...
    return _partition_helper(partitioner, cfg, target_label_col, fetch_only_test_data, subtask, train_dataset, test_dataset)
...
def _load_dist_based_clients_server_datasets(cfg, dat_partitioner_func, fetch_only_test_data=False):
    ...
    if cfg.dname in ["cifar10", "cifar100", "mnist"]:
        ...
        return _initialize_medical_dataset(cfg, dat_partitioner_func, fetch_only_test_data)
