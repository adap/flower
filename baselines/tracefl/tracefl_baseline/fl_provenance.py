...
from .utils import normalize_contributions, get_prov_eval_metrics
from .model import test_neural_network


class FlowerProvenance:
    def __init__(self, cfg, round_id, server_test_data, global_model, client2model, client2num_examples, client2class):
        """
        ...
        self.ALLROUNDSCLIENTS2CLASS = {
            client_id: self.client2class.get(client_id, {}) for client_id in self.client2model.keys()
        }
        self.participating_clients_labels = []  # Will be set in _setParticipatingClientsLabels
        self._setParticipatingClientsLabels()
        
        # Select provenance data
        self._selectProvenanceData()
    ...
    def _setParticipatingClientsLabels(self):
        """
        ...
        logging.debug(f"participating_clients_labels: {self.participating_clients_labels}")
    ...
    def _select_faulty_provenance_data(self, min_per_label: int = 10) -> None:
        ...
        wrong_indices, actual_labels, predicted_labels = self._eval_and_extract_wrong_preds(
            self.server_test_data
        )

        logging.info(f"Total wrong predictions: {len(wrong_indices)}")
        logging.info(f"label2flip mapping: {self.label2flip}")
        ...
        if not selected_wrong_indices and wrong_indices:
            ...
            logging.warning(
                "No misclassifications matched label2flip mapping; using first %s incorrect samples as fallback",
                fallback_count,
            )
            selected_wrong_indices = [int(idx) for idx in wrong_indices[:fallback_count]]

        logging.info(f"Selected wrong indices: {selected_wrong_indices}")
        ...
        if selected_wrong_indices:
            ...
            logging.info(f"Selected actual labels: {selected_actual}")
            logging.info(f"Selected predicted labels: {selected_pred}")
        else:
            logging.info("No incorrect predictions available for provenance analysis")

    def _selectProvenanceData(self, min_per_label: int = 2):
        """
        Select a subset of test data for provenance analysis based on balanced, correctly predicted samples.
        This matches the FederatedProvTrue logic from TraceFL-main.
        ...
        correct_indices, actual_labels = self._eval_and_extract_correct_preds(self.server_test_data)

        logging.info(f"Total correct predictions: {len(correct_indices)}")

        # Balance dataset by label (same as TraceFL-main)
        balanced_indices = self._balance_dataset_by_label(correct_indices, actual_labels, min_per_label)
        ...
        if len(balanced_indices) > 0:
            ...
            if hasattr(self.server_test_data, 'select'):
                ...
                self.subset_test_data = torch.utils.data.Subset(self.server_test_data, balanced_indices)

            logging.info(f"Selected {len(self.subset_test_data)} balanced samples for provenance analysis")
        else:
            ...
            if hasattr(self.server_test_data, 'select'):
                ...
                self.subset_test_data = torch.utils.data.Subset(self.server_test_data, range(subset_size))

            logging.info(f"Using {subset_size} random samples as fallback")
    ...
    def _eval_and_extract_wrong_preds(self, test_data):
        ...
        if hasattr(self.global_model, 'config'):
            arch = "transformer"
        elif any(name.startswith('resnet') for name in [type(self.global_model).__name__]):
            arch = "resnet"
        elif any(name.startswith('densenet') for name in [type(self.global_model).__name__]):
            arch = "densenet"
    ...
    def _eval_and_extract_correct_preds(self, test_data):
        """
        Evaluate the model on test data and extract indices and labels for correct predictions.
        This matches the FederatedProvTrue logic from TraceFL-main.
    ...
    def _balance_dataset_by_label(self, correct_indices, dataset_labels, min_per_label):
        ...
        This matches the _balanceDatasetByLabel logic from TraceFL-main.
    ...
    def calculate_client_contributions(self):
        ...
        try:
            ...
            input2prov = neuron_prov.computeInputProvenance()
            ...
            top_contributor = max(normalized_contributions, key=normalized_contributions.get)
            ...
            results = {
                ...
                "avg_prov_time_per_input": (end_time - start_time) / len(self.subset_test_data),
            }
            ...
            import traceback
            logging.error(f"Error in provenance calculation: {e}")
    ...
    def _aggregate_client_contributions(self, input2prov):
        ...
        """
        client_contributions = {client_id: 0.0 for client_id in self.client2model.keys()}
    ...
    def _compute_eval_metrics(self, input2prov):
        ...
        # Extract traced clients and actual clients
        traced_clients = [prov["traced_client"] for prov in input2prov]
        
        # For simplicity, assume we're evaluating against ground truth
        # In a real scenario, you would compare against known faulty clients
        actual_clients = list(self.client2model.keys())
        
        # Calculate accuracy (simplified)
        correct_traces = sum(1 for traced in traced_clients if traced in actual_clients)
        accuracy = correct_traces / len(traced_clients) if traced_clients else 0.0
    ...
    def _computeEvalMetrics(self, input2prov):
        """
        Compute evaluation metrics based on provenance information.
        This matches the _computeEvalMetrics method from TraceFL-main.
        ...
        client2class = {c: self.ALLROUNDSCLIENTS2CLASS.get(c, {}) for c in self.client2model}
        ...
        for idx, prov_r in enumerate(input2prov):
            ...
            target_l = target_labels[idx] if idx < len(target_labels) else idx % self.cfg.data_dist.num_classes
            ...
            if self._faulty_mode:
                ...
                responsible_clients = [
                    cid for cid, c_labels in client2class.items() if target_l_str in c_labels
                ]
                ...
                if normalized_client_counts and target_l_str in normalized_client_counts:
                    logging.info(f"     Traced Client: c{traced_client} || Tracing = Correct")
                    correct_tracing += 1
                    predicted_labels.append(1)
                    true_labels.append(1)
