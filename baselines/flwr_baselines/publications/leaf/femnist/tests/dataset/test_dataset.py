...
from femnist.dataset.dataset import create_dataset, create_partition_list, partition_dataset, train_valid_test_partition
...
class TestDataset(unittest.TestCase):
    def test_partitioning(self):
        ...
        assert_that(len(partitioned_datasets), is_(equal_to(sampled_df.writer_id.unique().shape[0])))
        temp_dir.cleanup()
    ...
    def test_train_valid_test_div(self):
        ...
        assert_that(len(partitioned_train[0]), is_(equal_to(int(len(division_list[0]) * train_fraction))))
        temp_dir.cleanup()
