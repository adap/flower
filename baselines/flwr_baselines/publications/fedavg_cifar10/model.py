...
class Net(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        output_tensor = nn.functional.relu(output_tensor)
        output_tensor = nn.functional.max_pool2d(output_tensor, kernel_size=2)
        output_tensor = self.conv2(output_tensor)
        output_tensor = nn.functional.relu(output_tensor)
        output_tensor = nn.functional.max_pool2d(output_tensor, kernel_size=2)

        # Flatten for Fully Connected Layers
        output_tensor = output_tensor.view(-1, 64*8*8)
