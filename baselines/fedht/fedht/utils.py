...
def partition_data(data, num_partitions):
    ...
    # Create partitions
    partitionsX = [X[i * partition_size:(i + 1) * partition_size] for i in range(num_partitions)]
    partitionsy = [y[i * partition_size:(i + 1) * partition_size] for i in range(num_partitions)]

    # Handle any remaining items
    if len(data) % num_partitions != 0:
        # partitions[-1] = partitions[-1] + data[num_partitions * partition_size:]
        partitionsX[-1] = np.vstack((partitionsX[-1], X[num_partitions * partition_size:]))
        partitionsy[-1] = np.vstack((partitionsy[-1], y[num_partitions * partition_size:]))
