...
def generate_client_fn_simII(dataset, 
                             num_features,
                             num_classes,
                             model, 
                             cfg: DictConfig):
    ...
    def client_fn(context: Context) -> Client:
        ...
        test_dataset = train_dataset = MyDataset(X_train[int(partition_id),:,:], y_train[:,int(partition_id)])
        ...
        return SimIIClient(trainloader, testloader, model, num_obs, num_features, num_classes, cfg).to_client()
...
# client fn for input into simulation
def generate_client_fn_mnist(dataset, 
                             num_features,
                             num_classes,
                             model, 
                             cfg: DictConfig):
    ...
    def client_fn(context: Context) -> Client:
        ...
        # Load the partition data
        train_dataset = dataset.load_partition(int(partition_id), "train").with_format("numpy")
        num_obs = train_dataset.num_rows
        test_dataset = dataset.load_partition(int(partition_id), "train").with_format("numpy")
        trainloader = DataLoader(train_dataset, batch_size=cfg.batch_size, shuffle=True)
        testloader = DataLoader(test_dataset, batch_size=cfg.batch_size, shuffle=True)

        return MnistClient(trainloader, testloader, model, num_obs, num_features, num_classes, cfg).to_client()
