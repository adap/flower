...
def main(cfg: DictConfig):
    ...
    if cfg.data == 'mnist':
        ...
        # set client function
        client_fn = generate_client_fn_mnist(dataset, 
        ...
        # set client function
        client_fn = generate_client_fn_simII(dataset, 
                                            num_features=num_features,
                                            num_classes=num_classes,
                                            model=model,
                                            cfg=cfg)
    ...
    strategy_fedht = FedHT(min_available_clients=cfg.strategy.min_available_clients,
    ...
    strategy_fedavg = fl.server.strategy.FedAvg(min_available_clients=cfg.strategy.min_available_clients,
                                        evaluate_fn=get_evaluate_fn(testloader, model),
                                        on_fit_config_fn=fit_round,
                                        initial_parameters=init_params)
    ...
    filename = "simII_" + cfg.agg + iterstr + "_local" + str(cfg.num_local_epochs) + "_lr" + str(cfg.learning_rate) + "_numkeep" + str(cfg.num_keep) + ".pkl" 
