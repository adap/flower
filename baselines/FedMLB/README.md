...
labels: [data heterogeneity, knowledge distillation, image classification] # please add between 4 and 10 single-word (maybe two-words) labels (e.g. "system heterogeneity", "image classification", "asynchronous", "weight sharing", "cross-silo")
...
First ensure you have activated your Poetry environment (execute `poetry shell` from
this directory).
...
are for CIFAR-100 with (a) Moderate-scale with Dir(0.3), 100 clients, 
5% participation, and (b) large-scale experiments have 500 clients with 2% participation rate.
To reproduce (a) run the following:
...
To reproduce (b) run the following:
```bash
