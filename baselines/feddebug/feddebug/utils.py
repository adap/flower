...
def set_exp_key(cfg):
    ...
    key = (
        f"{cfg.model_name}-{cfg.dataset.name}-"
...
def plot_metrics(gm_accs, feddebug_accs, cfg):
    ...
    title = f"{cfg.dist_type} - {cfg.model_name} - {cfg.dataset.name}"
    ...
    ax.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))  # Show integer values for rounds
    ...
    plt.savefig(f"graphs/{title}.png", dpi=300, bbox_inches='tight')
    plt.close()
