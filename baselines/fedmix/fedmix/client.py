...
class FedMixClient(NumPyClient):
    ...
    def local_update(self):
        ...
        ), lr=self.learning_rate * self.lr_decay_accumulated)
        self.net.train()
    ...
    def training_loop(self, criterion, optimizer):
        for images, labels in self.trainloader:
            ...
            mashed_image, mashed_label = mashed_image[None, :].to(
                self.device), mashed_label[None, :].to(self.device)
            ...
            l3 = self.mixup_ratio * \
                torch.inner(gradients.flatten(start_dim=1),
                            mashed_image.flatten(start_dim=1))
            l3 = torch.mean(l3)
...
def gen_client_fn(config, trainloaders, model):
    def client_fn(cid):
        ...
        return FedMixClient(net, trainloader, device, config.num_local_epochs, config.lr)
