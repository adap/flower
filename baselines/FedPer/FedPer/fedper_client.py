...
from FedPer.model import DecoupledModel, train, test
from FedPer.utils import MEAN, STD
from FedPer.dataset_preparation import DATASETS

from FedPer.fedavg_client import FlowerClient as FedAvgFlowerClient
...
class FedPerClient(
    ...
    def __init__(
        ...
        self.train_id = 1
        self.finetune_epochs = finetue_epochs

        self.personal_params_name = [
            name for name in self.net.state_dict().keys() if 'classifier' in name
        ]

        self.init_personal_params_dict = {
            name: param.clone().detach() 
            for name, param in self.net.state_dict(keep_vars=True).items()
    ...
    def get_parameters(self, config: Dict[str, Scalar]) -> NDArrays:
        ...
        return [val.cpu().numpy() for _, val in self.net.base.state_dict().items()]
    ...
    def set_parameters(self, parameters: NDArrays) -> None:
        ...
        model_keys = [k for k in self.net.state_dict().keys() if k.startswith("base")]
