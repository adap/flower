...
from feddefender.client_app import CNNFlowerClient
from feddefender.dataset import ClientsAndServerDatasets
from feddefender.model import initialize_model, test
from feddefender.strategy import FedAvgWithFedDefender
...
def _fit_metrics_aggregation_fn(metrics):
    ...
    for k in sorted(all_logs.keys()):
        log(INFO, all_logs[k])
    return {"loss": 0.0, "accuracy": 0.0, "asr": 0.0}
...
def run_simulation(cfg):
    ...
    def _callback_fed_defender_evaluate_fn(server_round, predicted_malicious_clients):
        true_malicious_clients = cfg.malicious_clients_ids

        log(INFO, "***FedDefender Output Round %s ***", server_round)
        log(INFO, "True Malicious Clients (Ground Truth) = %s", true_malicious_clients)
        log(INFO, "Total Random Inputs = %s", cfg.feddefender.r_inputs)
        localization_accuracy = utils.calculate_localization_accuracy(
            true_malicious_clients, predicted_malicious_clients
        )

        mal_probs = {
            c: v / cfg.feddefender.r_inputs for c, v in predicted_malicious_clients.items()
        }
        log(INFO, "Predicted Malicious Clients = %s", mal_probs)
        log(INFO, "FedDefender Localization Accuracy = %s", localization_accuracy)
        round2feddefender_accs.append(localization_accuracy)

    initial_net = _create_model()
    strategy = FedAvgWithFedDefender(
        num_bugs=num_bugs,
        num_inputs=cfg.feddefender.r_inputs,
        input_shape=server_testdata.dataset[0]["image"].clone().detach().shape,
        na_t=cfg.feddefender.na_t,
        device=cfg.device,
        fast=cfg.feddefender.fast,
        callback_create_model_fn=_create_model,
        callback_fed_defender_evaluate_fn=_callback_fed_defender_evaluate_fn,
    ...
    utils.plot_metrics(round2gm_accs, round2asr, cfg, save_path)
