...
class NumNodesGroupedNaturalIdPartitioner(GroupNaturalIdPartitioner):
    ...
    def load_partition(self, node_id: int) -> datasets.Dataset:
        ...
        if self._num_nodes <= node_id:
             raise KeyError(f'There are only {self._num_nodes} nodes [0-{self._num_nodes - 1}], {node_id} is out of this range. ')
        ...
        dataset =  dataset.shard(
            num_shards=int(group_sizes[num_group]), index=position_in_group, contiguous=True
            )
