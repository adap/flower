...
class DirichletPartitioner(Partitioner):
    ...
    def __init__(self, num_partitions:int, alpha: Union[float, List[float], NDArrayFloat],
                 partition_by: str, min_require_size: Optional[int] = None, self_balancing:bool=True ):
        super().__init__()
    ...
    def load_partition(self, node_id: int) -> datasets.Dataset:
        ...
        if not self._dataset_partioned:
            ...
            targets = self.dataset[self._partition_by]
            while True:
                ...
                for k in self._unique_classes:
                    ...
                    for nid in range(self._num_partitions):
                        nid_to_proportion_of_k_samples[nid] = class_k_division_proportion[nid]

                    # Balancing (not mentioned in the paper)
                    if self._self_balancing:
                        for nid, proportion in nid_to_proportion_of_k_samples.items():
                            # if from the previous classes you got too much samples don't add any further
                            if len(nid_to_indices[nid]) > self._avg_num_of_samples_per_node:
                                nid_to_proportion_of_k_samples[nid] = 0

                        # Renormalize such that p sums to 1
                        # (apply this opperation even if htere was no change; it won't modify the values)
                        sum_proportions = nid_to_proportion_of_k_samples.values().sum()
                        for nid, proportion in nid_to_proportion_of_k_samples.items():
                            nid_to_proportion_of_k_samples[nid] = proportion/ sum_proportions

                    # Determine the split indices
                    # Drop the last one (it adds up to the total number of samples (todo: check if it does)
                    indices_on_which_split = (np.cumsum(nid_to_proportion_of_k_samples.values()) * len(indices_representing_class_k)).astype(int)[:-1]
                    ...
                    for nid in nid_to_indices:
                        nid_to_indices[nid].extend(split_indices)

                # Determine if the assignements meet hte min_samples_size_per_node requriement
                # If not repeat the process
                # Otherwise break the while infinite loop
                min_sample_size_on_client = min(len(indices) for indices in nid_to_indices.values())
                if min_sample_size_on_client >= self._min_require_size:
                    break

            # Shuffling the indices not to have the samples per class in sequences [00000, 11111 etc)
    ...
    def _determine_basic_info(self):
        # Determine the basic information that are needed for dirichlet partiioner to start
        ...
        if isinstance(self._alpha, float):
            self._alpha = np.array(self._alpha, dtype=np.float).repeat(self._num_unique_classes)
