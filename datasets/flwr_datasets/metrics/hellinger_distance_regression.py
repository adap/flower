...
# ==============================================================================
"""HellingerDistanceRegression function to calculate Hellinger distance for regression
tasks.
"""
...
def HellingerDistanceRegression(
    targets_per_client: List[List[Union[int, float]]], num_bins: int = 20
) -> float:
    """Calculate the Hellinger distance for multiple numerical clients' targets (i.e.,
    regression tasks).
    ...
    from flower.datasets.flwr_datasets.metrics.hellinger_distance_regression import HellingerDistanceRegression

    random_targets_lists = np.random.randint(low=0, high=20, size=(10, 100)).tolist()

    HD = HellingerDistanceRegression(random_targets_lists, num_bins=10)
