...
class ObjectStoreTest(unittest.TestCase):
    ...
    def test_put_and_get(self) -> None:
        ...
        object_store.preregister(run_id=110, object_ids=[object_id])
    ...
    def test_clear(self) -> None:
        ...
        object_store.preregister(run_id=110, object_ids=[object_id1])
        object_content2 = CustomDataClass(b"test_value2").deflate()
        object_id2 = get_object_id(object_content2)
        object_store.preregister(run_id=110, object_ids=[object_id2])
    ...
    def test_preregister(self) -> None:
        ...
        not_present = object_store.preregister(
            run_id=110, object_ids=[object_id1, object_id2]
        )
        ...
        not_present = object_store.preregister(run_id=110, object_ids=[object_id3])
    ...
    def test_preregister_with_invalid_object_id(self, invalid_object_id) -> None:
        ...
        with self.assertRaises(ValueError):
            object_store.preregister(run_id=110, object_ids=[invalid_object_id])
    ...
    def test_put_get_delete_object_with_children(self) -> None:
        ...
        _, id_to_content = _create_object_hierarchy()
        ids = list(id_to_content.keys())

        # Execute: Preregister and put all objects
        object_store = self.object_store_factory()
        object_store.preregister(run_id=110, object_ids=ids)
    ...
    def test_delete_objects_in_run(self) -> None:
        ...
        object_store = self.object_store_factory()
        object_store.preregister(run_id=1, object_ids=ids[:4])

        # Execute: Preregister parent 2 and its descendants for run 2
        object_store.preregister(run_id=2, object_ids=[ids[2], ids[4]])
